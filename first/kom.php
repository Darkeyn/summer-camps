  ...
  if( isset( $_POST['smena'] ) )
{
    ...
    {
        case 'one':
        $mesta = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 1 AND `Places` = 0");
        $user = $mesta->fetch_assoc();
            ...
            break;
        case 'two':
            $mesta = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 2 AND `Places` = 0");
            $user = $mesta->fetch_assoc();
                if (count($user) != 0) {
                  $error_mesta = " В выбранной смене нет свободных мест!";
                  $error = true;
            ...
            break;
        case 'three':
        $mesta = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 3 AND `Places` = 0");
        $user = $mesta->fetch_assoc();
            if (count($user) != 0) {
              $error_mesta = " В выбранной смене нет свободных мест!";
              $error = true;
  ...
  if (!$error) {
    ...
    $mysqli->query("INSERT INTO `bron` (`Name_lager`, `Pochta_rod`, `Name_reb`, `Date`, `Sum`,`Id_rod`,`Id_lager`,`Id_reb`, `Smena`) VALUES ('Комсомольский', '$pochta_rod', '$fio_reb', '$date', '20000','$Id','3','$Id','$smenanumber')");
      ...
      <center>
       ...
       <div class="img_block" style="
         ...
         function printlager ($lagerinfo){
             ...
             $lagerinfo = $mysqli->query("SELECT * FROM `lager` WHERE `Name` LIKE 'Комсомольский'");
         ...
         <?php
          ...
          $dir = "img/kom"; // Путь к директории, в которой лежат изображения
        ...
        <center>
         ...
         <?php  if ($_COOKIE['proverka'] == ''):?>
           ...
           $mysqli = new mysqli("localhost","admin_bd","2000","bd_lager");
           $mysqli->query( "SET NAMES 'utf8'");
           $one = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 1");
           $two = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 2");
           $three = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 3");
