  ...
  if( isset( $_POST['smena'] ) )
{
    ...
    {
        case 'one':
        $mesta = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 4 AND `Places` = 0");
        $user = $mesta->fetch_assoc();
            ...
            break;
        case 'two':
            $mesta = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 5 AND `Places` = 0");
            $user = $mesta->fetch_assoc();
                if (count($user) != 0) {
                  $error_mesta = " В выбранной смене нет свободных мест!";
                  $error = true;
            ...
            break;
        case 'three':
        $mesta = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 6  AND `Places` = 0");
        $user = $mesta->fetch_assoc();
            if (count($user) != 0) {
              $error_mesta = " В выбранной смене нет свободных мест!";
              $error = true;
  ...
  if (!$error) {
    ...
    $mysqli->query("INSERT INTO `bron` (`Name_lager`, `Pochta_rod`, `Name_reb`, `Date`, `Sum`,`Id_rod`,`Id_lager`,`Id_reb`, `Smena`) VALUES ('Солнышко', '$pochta_rod', '$fio_reb', '$date', '15000','$Id','1','$Id','$smenanumber')");
...
$(this).fadeOut();
      ...
      <center>
       ...
       <div class="img_block" style="
         ...
         function printlager ($lagerinfo){
             ...
             $lagerinfo = $mysqli->query("SELECT * FROM `lager` WHERE `Name` LIKE 'Солнышко'");
             printlager($lagerinfo);
             $mysqli->close();
         ?>
           <p style="font-family: Comic Sans MS, sans-serif;text-indent: 0px">Возрастное ограничение: 7-10 лет</p>
  ...
  $dir = "img/sol"; // Путь к директории, в которой лежат изображения
...
<center>
  ...
  <p style="font-size: 1.7em; font-family: Comic Sans MS, sans-serif;text-indent: 0px">Бронирование:</p>
         ...
         <?php  if ($_COOKIE['proverka'] == ''):?>
             ...
             $mysqli = new mysqli("localhost","admin_bd","2000","bd_lager");
             $mysqli->query( "SET NAMES 'utf8'");
             $one = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 4");
             $two = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 5");
             $three = $mysqli->query("SELECT * FROM `smena` WHERE `Id` = 6");
