...
if ($_POST["prava"]) {
  ...
  if (!$error && count($user) != 0){
    $mysqli->query("UPDATE `users` SET `Prava` = 'Admin' WHERE `users`.`Id` = '$number';");
  }
  $mysqli->close();
}
 ?>
...
<html>
    ...
    <body>
      ...
      <form class="" action="" method="post">
        <center>
        <p style="font-family: Comic Sans MS, sans-serif;text-indent: 0px">Введите ID пользователя, которому хотите дать права администратора:</p>
        <input type="text" name="number" style="margin-top: 10px;"/>
        <span style="color:red"><?=$error_udal?></span><br />
        <input type="submit" name="prava" value="Предоставить права" style="
          font-size: 1.2em;
